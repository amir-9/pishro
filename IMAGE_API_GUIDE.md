...
تصاویر روی **S3 Object Storage** ذخیره می‌شوند:
- **مسیر ذخیره**: `images/{category}/{filename}`
- **URL دسترسی**: `{S3_PUBLIC_URL}/images/{category}/{filename}`
- **نوع دسترسی**: عمومی (Public)
- **نیاز به احراز هویت**: خیر (فقط برای آپلود و مدیریت نیاز است)

✅ **مزایا**:
- سازگار با محیط‌های serverless (Vercel, AWS Lambda)
- مقیاس‌پذیر و امن
- سرعت بالا با CDN
...
4. ✅ تصاویر روی **S3 Object Storage** ذخیره می‌شوند در `images/{category}/`
5. ✅ URL تصاویر به صورت عمومی در دسترس هستند: `{S3_PUBLIC_URL}/images/{category}/{filename}`
6. ✅ حذف تصویر، فایل را از S3 و دیتابیس حذف می‌کند
7. ✅ دسته‌بندی به مدیریت بهتر تصاویر کمک می‌کند
8. ⚠️ **نیاز به تنظیمات S3**: باید متغیرهای محیطی S3 را در `.env` تنظیم کنید (مشابه ویدیوها)
